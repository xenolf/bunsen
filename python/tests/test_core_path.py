"""Tests for the four-tier bunsen-core binary discovery in `_core_path`."""
...
def core_path_module(monkeypatch):
    """Provide a fresh import of `bunsen._core_path` with BUNSEN_CORE_BIN cleared.

    `conftest.py` sets `BUNSEN_CORE_BIN` for the rest of the suite (pointing at
    ...
    needs.
    """
    monkeypatch.delenv("BUNSEN_CORE_BIN", raising=False)
    if "bunsen._core_path" in sys.modules:
        importlib.reload(sys.modules["bunsen._core_path"])
    import bunsen._core_path as cp
    return cp
...
def test_env_var_overrides_everything(core_path_module, monkeypatch, tmp_path):
    """Tier 1: BUNSEN_CORE_BIN wins even if a package-relative binary exists."""
    pkg_dir = Path(core_path_module.__file__).parent
    pkg_bin = pkg_dir / "bin" / "bunsen-core"
    _make_exec(pkg_bin)
    try:
        monkeypatch.setenv("BUNSEN_CORE_BIN", "/custom/path/bunsen-core --flag")
        argv = core_path_module.find_core_bin()
        assert argv == ["/custom/path/bunsen-core", "--flag"]
    finally:
...
def test_package_relative_tier_returns_bundled_binary(core_path_module):
    """Tier 2 (new): `<bunsen package>/bin/bunsen-core` is returned when present."""
    pkg_dir = Path(core_path_module.__file__).parent
    pkg_bin = pkg_dir / "bin" / "bunsen-core"
    _make_exec(pkg_bin)
    try:
...
def test_workspace_target_release_tier_returns_dev_build(core_path_module, tmp_path, monkeypatch):
    """Tier 3 (unchanged): walks up from the package and finds `target/release/bunsen-core`.
    ...
    parent-walk locates `target/release/bunsen-core` after `cargo build --release`.
    """
    ...
    target_bin = repo_root / "target" / "release" / "bunsen-core"
    if not (target_bin.is_file() and os.access(target_bin, os.X_OK)):
        pytest.skip("target/release/bunsen-core not built — covered by integration env")
...
def test_path_tier_used_when_no_other_match(core_path_module, tmp_path, monkeypatch):
    ...
    fake_bin = tmp_path / "bunsen-core"
    _make_exec(fake_bin)
    monkeypatch.setenv("PATH", str(tmp_path))

    # Suppress tier 3 by making the parent-walk find nothing relevant.
    # We rely on `repo_root / target / release / bunsen-core` not existing in
    # tmp_path's parents — true by construction.
    fake_module_root = tmp_path / "fakepkg" / "bunsen"
    ...
    sys.modules.pop("bunsen", None)
    sys.modules.pop("bunsen._core_path", None)
    try:
        cp_iso = importlib.import_module("bunsen._core_path")
...
def test_no_match_raises_filenotfound(core_path_module, tmp_path, monkeypatch):
    ...
    try:
        ...
        assert "BUNSEN_CORE_BIN" in msg
        assert "pip install" in msg or "wheel" in msg or "bunsen/bin" in msg
