...
**v1 has no native macOS path.** Bunsen's sandbox provider is Firecracker, which needs the Linux KVM API. macOS does not expose KVM, and Apple's Hypervisor.framework is not a drop-in substitute. The only supported workflow on a Mac is to SSH to a remote Linux box with nested virtualization and develop there.
...
- Some primitives bunsen depends on (nftables, ext4 rootfs images, journalctl, /sbin/init bring-up via a custom guest kernel) are Linux-specific.
...
Intel Mac users *can* in theory enable nested KVM inside a Linux VM (Lima, Multipass, UTM) and run bunsen there, but no recipe is shipped and the path is not exercised in CI. Treat it as unsupported.
...
# 6. Clone and build bunsen (see README.md).
...
```sh
sudo setcap 'cap_net_bind_service,cap_net_admin=+ep' target/release/bunsen-core
```

(`cap_net_admin` is what lets bunsen drive nftables / TAP devices unprivileged.)

### Working from a Mac

Either:

- **SSH + a remote IDE.** VS Code Remote-SSH, JetBrains Gateway, and Cursor all work — bunsen builds with `cargo` and edits like any other Rust + Python project. Run `cargo test` and `pytest` on the remote side.
- **rsync + `ssh -t`.** If you prefer a local editor, sync your tree with `rsync -avz --exclude target ./ user@host:bunsen/` and invoke `cargo`/`pytest` over SSH.
