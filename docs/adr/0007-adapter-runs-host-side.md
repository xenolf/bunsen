...
The Adapter runs in the `crucible-core` host process, parsing raw stdout/stderr bytes that arrive over vsock from the guest. The guest-side init binary is a small static Rust binary that does PID 1 work, exec's the agent, and pipes raw bytes up the vsock — it has no Adapter-specific knowledge.
...
- **Rootfs simplicity.** Per-Adapter OCI images (ADR-0008) need to ship the agent binary and its runtime. Adding the Adapter Rust binary on top blurs the distribution boundary — the rootfs becomes "agent stuff + crucible stuff" instead of "agent stuff and a tiny init".
...
- The guest init binary stays tiny (~200 lines) and is built/distributed once per crucible-core release, decoupled from Adapter changes.
