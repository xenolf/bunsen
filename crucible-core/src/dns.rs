...
// ─── Slice 10p: host /etc/resolv.conf parser ─────────────────────────────

/// Parse the first valid `nameserver` directive out of an `/etc/resolv.conf`
/// body and return it as a [`SocketAddr`] suitable for use as the L7-side
/// upstream DNS resolver (slice 10m).
///
/// `resolv.conf` syntax (RFC notwithstanding, glibc's behaviour is the de
/// facto contract): each line is either blank, a comment (`#` or `;`
/// leading), or a directive. The directive we care about is
/// `nameserver <IPv4 or IPv6 literal>`. The first valid `nameserver` wins
/// — that matches what glibc's resolver does on a fresh process (the second
/// and third lines are fallbacks).
///
/// Lines whose IP literal fails to parse are skipped, not treated as a hard
/// error: a typo in `/etc/resolv.conf` shouldn't take the Run down. Returns
/// `None` when the file is empty or contains no valid `nameserver` lines.
///
...
pub fn parse_resolv_conf(contents: &str) -> Option<SocketAddr> {
    for line in contents.lines() {
        ...
        // these, but other resolv.conf consumers do, so being tolerant here
        // costs nothing.
        ...
        if !directive.eq_ignore_ascii_case("nameserver") {
            continue;
        }
        let Some(ip_str) = tokens.next() else {
            continue;
        };
        match ip_str.parse::<std::net::IpAddr>() {
            Ok(ip) => return Some(SocketAddr::new(ip, 53)),
            Err(_) => continue,
        }
    }
    None
...
/// Resolve the default upstream DNS resolver for the L7 path: prefer the
/// host's `/etc/resolv.conf`, fall back to a Google Public DNS literal so
...
pub fn default_dns_upstream() -> SocketAddr {
    const FALLBACK: &str = "8.8.8.8:53";
    if let Ok(contents) = std::fs::read_to_string("/etc/resolv.conf") {
        if let Some(addr) = parse_resolv_conf(&contents) {
            return addr;
        }
    }
    FALLBACK.parse().expect("static literal")
}
...
mod tests {
    ...
    // ── parse_resolv_conf (slice 10p) ──────────────────────────────────────

    #[test]
    fn parse_resolv_conf_picks_first_nameserver() {
        let body = "\
            ...
            options edns0\n";
        let got = parse_resolv_conf(body).expect("must find a nameserver");
        assert_eq!(got, "192.0.2.53:53".parse::<SocketAddr>().unwrap());
    }

    #[test]
    fn parse_resolv_conf_handles_ipv6_nameserver() {
        let body = "nameserver fe80::1\n";
        let got = parse_resolv_conf(body).expect("must find a nameserver");
        assert_eq!(got, "[fe80::1]:53".parse::<SocketAddr>().unwrap());
        assert!(got.is_ipv6());
        assert_eq!(got.port(), 53);
    }

    #[test]
    fn parse_resolv_conf_skips_empty_and_comments() {
        let body = "\
            \n\
            # leading comment\n\
            ; semicolon comment\n\
            ...
            nameserver 1.1.1.1\n";
        let got = parse_resolv_conf(body).expect("must find a nameserver");
        assert_eq!(got, "1.1.1.1:53".parse::<SocketAddr>().unwrap());
    }

    #[test]
    fn parse_resolv_conf_strips_trailing_comment() {
        let body = "nameserver 9.9.9.9   # quad9\n";
        let got = parse_resolv_conf(body).expect("must find a nameserver");
        assert_eq!(got, "9.9.9.9:53".parse::<SocketAddr>().unwrap());
    }

    #[test]
    fn parse_resolv_conf_returns_none_for_no_nameserver() {
        let body = "\
            search lan\n\
            options edns0\n\
            domain example.com\n";
        assert!(parse_resolv_conf(body).is_none());
    }

    #[test]
    fn parse_resolv_conf_returns_none_for_empty_input() {
        assert!(parse_resolv_conf("").is_none());
        assert!(parse_resolv_conf("   \n\t\n").is_none());
    }

    #[test]
    fn parse_resolv_conf_skips_malformed_addresses() {
        // First line has a non-IP value; second line is valid. Don't refuse
        // the whole file just because one line is broken.
        let body = "\
            nameserver not-an-ip\n\
            nameserver 8.8.4.4\n";
        let got = parse_resolv_conf(body).expect("must find a nameserver");
        assert_eq!(got, "8.8.4.4:53".parse::<SocketAddr>().unwrap());
    }

    #[test]
    fn parse_resolv_conf_directive_is_case_insensitive() {
    ...
    #[test]
    fn parse_resolv_conf_requires_a_value() {
        let body = "nameserver\nnameserver 1.0.0.1\n";
        let got = parse_resolv_conf(body).expect("must find a nameserver");
        assert_eq!(got, "1.0.0.1:53".parse::<SocketAddr>().unwrap());
    }

    #[test]
    fn parse_resolv_conf_ignores_extra_tokens_after_ip() {
        // glibc takes the first whitespace-separated token after nameserver.
        // Stray tokens (a port literal, or a comment without the # prefix)
        // are silently dropped at the parse step rather than failing the line.
        let body = "nameserver 1.1.1.1 53\n";
        let got = parse_resolv_conf(body).expect("must find a nameserver");
        assert_eq!(got, "1.1.1.1:53".parse::<SocketAddr>().unwrap());
    }

    #[test]
    fn parse_resolv_conf_handles_loopback_systemd_resolved() {
        // systemd-resolved's stub listener. We respect it: the L7 DNS
        // listener binds on net.host (link-local), so forwarding to
        // 127.0.0.53 on the host won't create a loop.
        let body = "nameserver 127.0.0.53\noptions edns0 trust-ad\nsearch .\n";
        let got = parse_resolv_conf(body).expect("must find a nameserver");
        assert_eq!(got, "127.0.0.53:53".parse::<SocketAddr>().unwrap());
    }
}
