...
//!   6. Fork + exec the agent with setpgid(0,0)
...
fn fork_exec(spec: &InitSpec) -> (pid_t, RawFd, RawFd) {
    ...
    let envp: Vec<CString> = spec.env.iter()
        ...
        .chain(std::env::vars().map(|(k, v)| CString::new(format!("{k}={v}")).unwrap()))
        .collect();
...
/// Returns the agent's exit code (0 if killed by signal).
fn wait_for_agent(agent_pid: pid_t) -> i32 {
    loop {
        let mut status = 0i32;
